...
class Perceptron:
    ...
    def Normalize(self, dataSet, attributeMeans=[],attributeStds=[]):
    ...
    def constantFeature(self, dataSet):
        ...
        DataSet.columns = range(1, DataSet.shape[1] + 1)
        DataSet.insert(0, 0, 1)
        return DataSet

    def predictResult(self, x, weights):
    ...
    def fit(self, dataSet):
        ...
        for each in range(self.epochs):
            ...
            for i in range(X.shape[0]):
                error = Y[i] - self.predictResult(X[i], weights)
                if error != 0:
    ...
    def GetAccuracy(self, dataSet, weights):
        X = np.matrix(dataSet.iloc[:, :-1])
        Y = dataSet.iloc[:, -1]

        prediction = []
        for i in range(dataSet.shape[0]):
            prediction.append(self.predictResult(X[i], weights))
    ...
    def validate(self):
        ...
        for trainIndex, testIndex in self.kf.split(self.dataSet):
            ...
            accuracy = self.GetAccuracy(testDataSet, weights)
            accuracies.append(accuracy)

            dualEpoch, dualWeights = self.FitDual(trainDataSet)
            ...
            print("{}\t{}\t{}\t{}\t{}".format(fold, epoch, accuracy, dualEpoch,dualAccuracy))
            fold += 1
